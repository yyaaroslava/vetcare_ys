...
// import { createVisit } from '../../api/visits'; // Поки що закоментуємо, якщо ще не готові візити
import { getClients, getVets } from '../../api/auth';
import { getAnimals } from '../../api/animals';
import { Spinner, StatusBadge, EmptyState, showToast, ConfirmModal, speciesEmoji, Modal } from '../../components/ui';
...
export default function VetSchedule() {
  ...
  if (selectedDate) {
    ...
    if (timeFilter === 'past') filtered = filtered.filter(a => a.date < todayStr);
  }
    ...
    <div className="page-container">
                ...
                <div key={a.id} className="appointment-block" style={{padding: '16px 20px', borderLeft: '4px solid var(--orange)', marginBottom: 12}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap: 20}}>
                     <div style={{minWidth: 100}}>
                      <div style={{fontSize: 22, fontWeight: 900, color: 'var(--orange)', lineHeight: 1}}>{a.time?.slice(0,5)}</div>
      ...
      <div className="card mt-6">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>Дата</th><th>Час</th><th>Тварина</th><th>Власник</th><th>Опис</th><th>Статус</th></tr></thead>
            <tbody>
              {filtered.length === 0
                ? <tr><td colSpan={6} style={{textAlign:'center',padding:20,color:'var(--gray-400)'}}>Немає прийомів</td></tr>
                : filtered.map(a => (
                  <tr key={a.id}>
                    <td>{a.date}</td>
